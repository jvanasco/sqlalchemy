...
class NamingConventionTest(fixtures.TestBase, AssertsCompiledSQL):
    ...
    def test_ck_name_required(self):
        ...
        assert_raises_message(
            ...
            r'"ck_%\(table_name\)s_%\(constraint_name\)s", table "user", and '
            r'column\(s\) "\[\'data\'\]".',
            CheckConstraint,
            u1.c.data == "x",
        )
    ...
    def test_schematype_ck_name_boolean_not_on_name__no_type_ck(self):
        ...
        assert_raises_message(
            exc.InvalidRequestError,
            r'Naming convention "ck_\%\(table_name\)s_\%\(constraint_name\)s" '
            r'failed to apply to CHECK constraint on table "user" with column '
            r'"\[\'x\'\]" since the constraint has no name, and the '
            r'convention uses the "\%\(constraint_name\)s" token\. Consider '
            r'using the "type_ck" naming convention for type-bound CHECK '
            r"constraints so that no name is necessary\. "
            r"\(Background on this error at: http://sqlalche.me/e/14/f0f1\)",
    ...
    def test_schematype_ck_name_enum_not_on_name(self):
        ...
        u1 = Table(
            "user", m1, Column("x", Enum("a", "b", create_constraint=True))
        )
        # constraint is not hit
        eq_(
            [c for c in u1.constraints if isinstance(c, CheckConstraint)][
    ...
    @testing.skip_if(lambda: not util.py3k, "py2 requires enum34 package")
    def test_schematype_ck_enum_pep435_no_name(self):
        ...
        u1 = Table(
            ...
            m1,
            Column(
                "x", Enum(MyEnum, native_enum=False, create_constraint=True)
            ),
        )
        eq_(
            [c for c in u1.constraints if isinstance(c, CheckConstraint)][
    ...
    def test_schematype_ck_no_columns_on_constraint(self):
        """
        see Issue #4784 #issuecomment-516581616

        This will raise an error because there are no columns to name the
        constraint with::
        ...
        Unlike other tests, this will trigger immediately on the `Table()`
        call, because the constraint's name is not "deferred" no name.
        """
        m1 = MetaData(
        ...
        assert_raises_message(
            exc.InvalidRequestError,
            r'Naming convention "ck_\%\(table_name\)s_\%\(column_0_name\)s" '
            r"requires column 0 for a constraint, however the constraint does "
            r'not have that number of columns. Check table "user" and '
            r'column\(s\) "\[\]"',
            Table,
            "user",
            m1,
